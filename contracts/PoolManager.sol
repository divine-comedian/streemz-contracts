...
abstract contract PoolManager {
    ...
    constructor(address _distributionToken, address _gdav1Forwarder) {
        ...
        (bool _success, address _pool) = GDAv1Forwarder(_gdav1Forwarder).createPool(
            _distributionToken,
            address(this),
    ...
    function _getAllocation(address _member) internal view returns (Allocation memory, uint256) {
    ...
    function _setAllocation(address _member, Allocation memory _newAllocation) internal {
        ...
        for (uint256 i = 0; i < _allocation.granteeIds.length; i++) {
            if (granteeAddresses[_allocation.granteeIds[i]] != address(0)) {
                ...
                pool.updateMemberUnits(granteeAddress, pool.getUnits(granteeAddress) - _allocation.amounts[i]);
            }
        }
        ...
        _internalAllocations[_member] =
            InternalAllocation({granteeIds: granteeIdsArray, amounts: _newAllocation.amounts});
        ...
        for (uint256 i = 0; i < granteeIdsArray.length; i++) {
            ...
            pool.updateMemberUnits(granteeAddress, pool.getUnits(granteeAddress) + _newAllocation.amounts[i]);
        }
    }
