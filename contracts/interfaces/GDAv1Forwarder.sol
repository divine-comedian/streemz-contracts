...
interface GDAv1Forwarder {
     ...
     */
    function updateMemberUnits(address pool, address memberAddress, uint128 newUnits, bytes memory userData)
        external
     ...
     * @param userData User-specific data.
     */
    function claimAll(address pool, address memberAddress, bytes memory userData) external returns (bool success);
     ...
     */
    function distribute(address token, address from, address pool, uint256 requestedAmount, bytes memory userData)
        external
        returns (bool);
     ...
     * @return A boolean value indicating whether the distribution was successful.
     */
    function distributeFlow(address token, address from, address pool, int96 requestedFlowRate, bytes memory userData)
        external
        returns (bool);
     ...
     */
    function getNetFlow(address token, address account) external view returns (int96);
     ...
     */
    function getFlowDistributionFlowRate(address token, address from, address to) external view returns (int96);
     ...
     */
    function getPoolAdjustmentFlowRate(address pool) external view returns (int96);
     ...
     */
    function estimateFlowDistributionActualFlowRate(address token, address from, address to, int96 requestedFlowRate)
        external
        view
     ...
     */
    function estimateDistributionActualAmount(address token, address from, address to, uint256 requestedAmount)
        external
        view
     ...
     */
    function getPoolAdjustmentFlowInfo(address pool) external view returns (address, bytes32, int96);
}
